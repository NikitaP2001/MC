...
TEST_CASE(token, concat_strlit)
{
     ...
     uint8_t res[] = { 0x20, 0x66, 0x69, 0x72, 0x73, 0x74, 0x20, 
                       ...
                       0x6e, 0x65, 0x20, 0x20, 0x74, 0x68, 0x69, 
                       0x72, 0x64, 0x20, 0x6c, 0x69, 0x6e, 0x65, 0x20, 0x0 };
     ASSERT_TRUE(write_file(TFILE_NAME, test, LEN_OF(test)));
     enum mc_status status = pp_run(&pp, TFILE_NAME);
     ASSERT_TRUE(MC_SUCC(status));
...
TEST_CASE(token, concat_wstrlit)
{
     ...
     char test[] =  "L\"Ucn: \\U00001234 \\U00012345\""
                    "\"ucn: \\u5f12 \\u0024 \\u0512\"\n\n"
                    "\" \\a \\b \\\\ \\f \\n \\r \\t \\v \""
                    "\" \\' \\\" \\? \""
                    "\" \\123 \\x1f \"";
     uint8_t res[] = { 0x55, 0x63, 0x6e, 0x3a, 0x20, 0xe1, 0x88, 0xb4, 0x20, 0xf0, 0x92, 0x8d, 0x85, 0x75, 0x63, 0x6e, 0x3a, 0x20, 0xe5, 0xbc, 0x92, 0x20, 0x24, 0x20, 0xd4, 0x92, 0x20, 0x7, 0x20, 0x8, 0x20, 0x1b, 0x20, 0xc, 0x20, 0xa, 0x20, 0xd, 0x20, 0x9, 0x20, 0xb, 0x20, 0x20, 0x27, 0x20, 0x22, 0x20, 0x3f, 0x20, 0x20, 0x53, 0x20, 0x1f, 0x20, 0x0 };
     ASSERT_TRUE(write_file(TFILE_NAME, test, LEN_OF(test)));
     enum mc_status status = pp_run(&pp, TFILE_NAME);
     ASSERT_TRUE(MC_SUCC(status));
